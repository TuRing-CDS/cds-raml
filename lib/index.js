    ...
    constructor(file, output) {
        ...
        raml2obj.parse(this.fileTxt).then(function (raml) {
            ...
            var mkd = function (array, callback) {
                ...
                if (item) {
                    if (item.indexOf('{') != -1) {
                        item = item.substr(0,item.indexOf('{')-1);
                        array.push(item);
                ...
                for (var key in temp) {
                    ...
                    for (var sub in item) {
                        ...
                        text += item[sub].map(function (item, index) {
                            let requireds = [];
                            return `\r\n\t//${item.description}\r\n\tns('${item.method.toUpperCase()} ${sub}',new Api()${
                                Object.keys(item.queryParameters || {}).map(function (t) {
                                    if (item.queryParameters[t].required) {
                                        requireds.push(t);
                                    }
                                    return `.param('${t}'${item.queryParameters[t].description ? `/*${item.queryParameters[t].description.replace('\n', '')}*/` : ''},Api.Types.TrimString)`
                                }).join('')
                                }${
                                (item.allUriParameters || []).map(function (t) {
                                    if (t.required) {
                                        requireds.push(t.displayName);
                                    }
                                    return `.param('${t.displayName}',Api.Types.TrimString)`
                                }).join('')
